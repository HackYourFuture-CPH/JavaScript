...
const threeSecondButtonElement = document.querySelector('button.count');
let number = 0;
threeSecondButtonElement.addEventListener('click', function() {
    console.log(number);
    number += 1;
});

const fiveSecondButtonElement = document.querySelector('button.five');
fiveSecondButtonElement.addEventListener('click', function() {
    setTimeout(function() {
        console.log("Log in 5 seconds");
    }, 5000);
});
...
const domFullyLoadedCallback = function() {
    console.log("DOM fully loaded and parsed");
}

document.addEventListener("DOMContentLoaded", domFullyLoadedCallback);
...
req.addEventListener('load', function () {
    ...
    if (this.status === 200) {
        const peopleInSpaceOptions = JSON.parse(req.responseText);

        document.querySelector('.peopleInSpace').innerHTML = 'Number of people in space: ' + peopleInSpaceOptions.number;
        console.log(peopleInSpaceOptions);
    } else {
